...
let CallEndListener = class CallEndListener extends PrismListener {
    async run(interaction) {
        if (!(interaction.isButton() && interaction.customId == 'callAddUsers' && interaction.channel && interaction.guild && interaction.channel.type == ChannelType.GuildVoice))
            return;
            ...
            components: [
                new ActionRowBuilder()
                    ...
                    new UserSelectMenuBuilder()
                        .setCustomId('callAddUsersSelect')
                        .setMaxValues(5)
                ])
            ]
        }));
